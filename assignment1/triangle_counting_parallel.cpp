...
void triCalParallel(uintV start, uintV end, int thread_id)
{
    timer t1;
    ...
    std::cout << thread_id << ", " << triangle_local_count << ", " << time_taken << "\n";
    mtx.unlock();
}
...
void triangleCountParallel(Graph &g, uint n_workers)
{
    ...
    for (int i = 0; i < n_workers; i++)
    {
        ...
        vector_threads.push_back(std::thread(triCalParallel, start, end, i));
    }
