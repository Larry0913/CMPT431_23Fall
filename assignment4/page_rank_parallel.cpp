...
void pageRankParallel(Graph &g, int max_iterations, int global_rank, int world_size)
{
    uintV n = g.n_;
    uintE m = g.m_;
    ...
    for (int iter = 0; iter < max_iterations; iter++)
    {
        ...
        if (global_rank != 0)
        {
            ...
            MPI_Recv(pr_recv + startVector[global_rank] - 1, endVector[global_rank] - startVector[global_rank] + 1, PAGERANK_MPI_TYPE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        else
        {
            ...
            for (int i = 1; i < world_size; i++)
            {
                MPI_Send(pr_next + startVector[i] - 1, endVector[i] - startVector[i] + 1, PAGERANK_MPI_TYPE, i, 0, MPI_COMM_WORLD);
            }
        }
    ...
    if (global_rank != 0)
    {
        MPI_Send(&local_sum, 1, PAGERANK_MPI_TYPE, 0, 1, MPI_COMM_WORLD);
    }
    else
    {
        for (int i = 1; i < world_size; i++)
        {
            PageRankType recv_data;
            MPI_Recv(&recv_data, 1, PAGERANK_MPI_TYPE, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            total_page_rank += recv_data;
        }
        total_page_rank += local_sum;
    }
...
int main(int argc, char *argv[])
{
    ...
    case 0:
        ...
        break;
    case 1:
        pageRankParallel(g, max_iterations, global_rank, world_size);
        break;
    }
    MPI_Finalize();
    return 0;
